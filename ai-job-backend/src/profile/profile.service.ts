...
export class ProfileService {
  ...
  async updateMe(userId: number, data: UpdateProfileDto) {
    const createData = {
      ...
      targetDirection: data.targetDirection ?? data.targetDirections?.[0] ?? 'internet',
      targetDirections: (data.targetDirections ?? ['internet']) as unknown as Prisma.InputJsonValue,
    ...
    const updateData = {
      ...data,
      targetDirection: data.targetDirection ?? data.targetDirections?.[0],
      targetDirections: data.targetDirections as unknown as Prisma.InputJsonValue | undefined,
    };
