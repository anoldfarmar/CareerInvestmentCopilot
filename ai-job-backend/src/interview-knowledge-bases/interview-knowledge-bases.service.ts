...
export class InterviewKnowledgeBasesService {
  ...
  private readonly audioRootDir = process.env.AUDIO_UPLOAD_DIR ?? '/home/public/audio';
  ...
  async createManualRecord(userId: number, knowledgeBaseId: string, data: CreateManualRecordDto) {
    ...
    await this.activityService.incrementAudioUpload(userId, record.createdAt);
  ...
  async createAudioRecord(
    ...
    const storedAudio = file ? await this.saveAudioFile(userId, file) : undefined;
  ...
  async createAudioRecordPipeline(
    userId: number,
    knowledgeBaseId: string,
    data: CreateAudioRecordDto,
    file?: Express.Multer.File,
  ) {
    const record = await this.createAudioRecord(userId, knowledgeBaseId, data, file);
    const transcribed = await this.transcribeAudioRecord(userId, knowledgeBaseId, record.id, {
    ...
    return this.buildRecord(userId, knowledgeBaseId, record.id);
  }

  async createUserAudioRecordPipeline(
    userId: number,
    data: CreateAudioRecordDto,
    file?: Express.Multer.File,
  ) {
    const knowledgeBase = await this.ensureAudioReviewKnowledgeBase(userId);
    return this.createAudioRecordPipeline(userId, knowledgeBase.id, data, file);
  }
  ...
  private async saveAudioFile(userId: number, file: Express.Multer.File) {
    assertAudioUpload(file);

    const userFolder = `user-${userId}`;
    const userAudioDir = join(this.audioRootDir, userFolder);
    await mkdir(userAudioDir, { recursive: true });
    const extension = getAudioExtension(file);
    const filename = `${Date.now()}-${randomUUID()}${extension}`;
    await writeFile(join(userAudioDir, filename), file.buffer);

    return {
      filename,
      publicUrl: this.buildAudioPublicUrl(userFolder, filename),
    };
  }

  private buildAudioPublicUrl(userFolder: string, filename: string) {
    const publicBaseUrl = (
      process.env.ASR_PUBLIC_BASE_URL ??
